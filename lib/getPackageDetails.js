 ...
 */
function getPackageDetailsFromGitHub({ path, hash }) {
  const [owner, repo] = String(path).slice(1).replace(/\.git$/, '').split('/');
...
module.exports = function getPackageDetails(
  ...
  if (versionUrlObj.protocol) {
    if (versionUrlObj.host === 'github.com') {
      // TODO: cache result
      return getPackageDetailsFromGitHub(versionUrlObj).then(({
